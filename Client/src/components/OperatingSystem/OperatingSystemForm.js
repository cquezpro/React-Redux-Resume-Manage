...
export const OperatingSystemForm = ({ handleSubmit, pristine, reset, submitting, saveHandler, cancelHandler }) => {
  return (
    <form onSubmit={ handleSubmit(saveHandler) }>
      <div className='slds-modal__content'>
        <Field type="text" name={ constants.OPERATINGSYSTEMNAME } label={ constants.OPERATINGSYSTEM } placeholder={ constants.OPERATINGSYSTEM } component={ FieldInput } autoFocus />
...
const validate = values => {
  const errors = {};
  if (!values.OperatingSystemName) {
      errors.OperatingSystemName = constants.REQUIRED_MESSAGE + ' ' + constants.OPERATINGSYSTEM;
  }

  return errors;
};

OperatingSystemForm.propTypes = {
