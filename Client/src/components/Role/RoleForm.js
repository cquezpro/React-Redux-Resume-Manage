...
export const RoleForm = ({ handleSubmit, pristine, reset, submitting, saveHandler, cancelHandler }) => {
  return (
    <form onSubmit={ handleSubmit(saveHandler) }>
      <div className='slds-modal__content'>
        <Field type="text" name={ constants.ROLENAME } label={ constants.ROLE } placeholder={ constants.ROLE } component={ FieldInput } autoFocus />
...
const validate = values => {
  const errors = {};
  if (!values.RoleName) {
      errors.RoleName = constants.REQUIRED_MESSAGE + ' ' + constants.ROLE;
  }
