...
export const DatabaseForm = ({ handleSubmit, pristine, reset, submitting, saveHandler, cancelHandler }) => {
  return (
    <form onSubmit={ handleSubmit(saveHandler) }>
      <div className='slds-modal__content'>
        <Field type="text" name={ constants.DATABASENAME } label={ constants.DATABASE } placeholder={ constants.DATABASE } component={ FieldInput } autoFocus />
...
const validate = values => {
  ...
  if (!values.DatabaseName) {
      errors.DatabaseName = constants.REQUIRED_MESSAGE + ' ' + constants.DATABASE;
  }
