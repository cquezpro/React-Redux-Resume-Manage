...
const linkEditData = (cell, row, formatExtraData) => {
  return (
    <LinkEditComponent rowData={ row } editHandler={ formatExtraData } moduleUrl={ constants.PROJECTROLE_URL } />
  );
};
    ...
    return (
      <div>
        <BootstrapTable className='projectRoleGrid' data={ this.props.projectRoles } striped hover search pagination={ true } options={ options } searchPlaceholder={ constants.FILTER }>
          ...
          <TableHeaderColumn dataField='ProjectRoleName' dataSort={ true } width='110'>{ constants.PROJECTROLE }</TableHeaderColumn>
