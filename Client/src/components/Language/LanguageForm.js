...
export const LanguageForm = ({ handleSubmit, pristine, reset, submitting, saveHandler, cancelHandler }) => {
  return (
    <form onSubmit={ handleSubmit(saveHandler) }>
      <div className='slds-modal__content'>
        <Field type="text" name={ constants.LANGUAGENAME } label={ constants.LANGUAGE } placeholder={ constants.LANGUAGE } component={ FieldInput } autoFocus />
...
const validate = values => {
  ...
  if (!values.LanguageName) {
      errors.LanguageName = constants.REQUIRED_MESSAGE + ' ' + constants.LANGUAGE;
  }
